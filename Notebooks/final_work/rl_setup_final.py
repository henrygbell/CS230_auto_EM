...
def build_potential(system, crop_size, center_x, center_y, sampling):
...
def simulate_haadf(
    ...
    if simplified == True:
        defocus = 0.0
        if aberation_dict is not None:
            defocus = float(aberation_dict.get("C10", 0.0))

        sigma = abs(defocus) / 100  * 3# rough estimate: larger defocus -> larger probe size

        pot_arr = np.array(potential.array.sum(0))
        
        

        # apply gaussian filter to absolute potential (simple proxy for defocus blur)
        filtered = gaussian_filter(pot_arr, sigma=sigma)

        # normalize to [0,1]
        img = filtered.astype(np.float32)
...
simulation_parameters = {
    ...
    "positions": np.load("nanoparticle_coms.npy"),
    "system": system,
...
simulation_parameters["image_shape"] = (64, 64)
...
#         return np.clip(self.state + action * 0.1, a_min = -1, a_max = 1) # update state to new absolute parameters
...
class STEM_environment_history(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        if options and 'initial_state' in options:
            init = options['initial_state']
            # expected dict: {'system': ..., 'center_x': .., 'center_y': .., 'deterministic': True}
            self.set_initial_state(init.get('system'), init.get('center_x'), init.get('center_y'), init.get("defocus"))
        else:
            # Reset state randomly
            self.state = np.random.uniform(-50, 50) - 25  # Random initial defocus
            
            i_choice = np.random.randint(self.positions.shape[0])
            self.position_rand = self.positions[i_choice]
            
            self.center_x = self.position_rand[0] + int(np.random.rand(1) * 4 - 2)  # randomize starting offset
            self.center_y = self.position_rand[1] + int(np.random.rand(1) * 4 - 2)
            
            self.potential = build_potential(
                self.system, 
                self.crop_size, 
                self.center_x, 
                self.center_y, 
        ...
        self.gt_potential = build_potential(               
                self.system, 
                self.crop_size, 
                self.center_x, 
                self.center_y, 
                self.interpolation_sampling
        )
        
        self.detector = build_detector_haadf(
            self.inner_angle, 
            self.outer_angle
        )
        
        # Get initial observation with no action (action=0)
        obs = self._forward_model()
        self._last_metric = self._focus_metric(obs)
    ...
    def set_initial_state(self, center_x, center_y, system, defocus):
    ...
    def step(self, action):
        ...
        beta = 0.2  # action penalty weight
        ...
        if focus_metric >= self.best_sharpness:
            self.patience_count = 0
        elif np.abs(focus_metric - self.best_sharpness) <  tolerance:
            self.patience_count += 1

        if self.patience_count >= patience:
            terminated = True
            termination_reward = end_reward
        else:
            terminated = self.step_count >= self.max_steps
            termination_reward = 0
            
        self.best_sharpness = max(self._last_metric, focus_metric)
        
        #reward =  2*(-np.abs(self.state)/50 + 0.5) - 2 * beta * (abs(action_scalar) - 0.5) + termination_reward#focus_metric - beta * abs(action_scalar)
        reward = 2 * (focus_metric / 0.1/7 - 0.5) -  beta * abs(action_scalar) + termination_reward
    ...
    def _forward_model(self):
        ...
        aberration_dict = {"C10": self.state}
        ...
        img = simulate_haadf(
            ...
            self.detector,
            self.sampling, 
            30,  # semi_angle_cutoff fixed at 30 mrad
...
def deploy_autofocus(system, model_path, vec_stats_path=None, n_steps=200, center=None, device='cpu', deterministic=True):
    # build env and wrap same as training (Monitor -> VecNormalize)
    def make_env():
        env = STEM_environment_history(config=simulation_parameters)
        return Monitor(env)

    vec_env = DummyVecEnv([make_env])

    # load VecNormalize stats so observations/rewards match training
    if vec_stats_path and os.path.exists(vec_stats_path):
        vec_env = VecNormalize.load(vec_stats_path, vec_env)
        vec_env.training = False

    # Load model with env to ensure proper action space / normalization
    ...
    # Use reset(options=...) to set initial state (Gym >=0.26)
    init_options = {'initial_state': {'system': system, 'center_x': (center[0] if center else None),
                                      'center_y': (center[1] if center else None), 'deterministic': deterministic}}
    # Reset the vectorized env with the options (VecEnv reset may forward options differently)
    obs = vec_env.reset(options=init_options)
